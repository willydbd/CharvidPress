...
function ManageProductionStagesViewmodel() {
	...
	mvm.confirm_stage_deletion = function() {
		return confirm("Do you really want to delete this production stage? All information in it would be non-retrievable.");
	};
	...
	mvm.add_process = function() {
		...
		if (exists) {
			swal({title: "Hold on", text: "This production process exists already", type: 'info'});
			return;
		}
	...
	mvm.update_stage_form_submit = function() {
		if (mvm.processes().length == 0) {
			swal({title: "Hold on", text: "Add at least one production process to this stage", type: 'info'});
			return false;
		}
		return confirm("Continue to update this production stage?");
	};
...
function PendingJobsViewmodel(where) {
	...
	pvm.print_receipt = function() {
		...
		, function(data) {
			...
			$('.not-receipt').hide();
			$('.receipt').show();
			$('.pos').addClass('print-hidden');
			$('.main-container').addClass('print-receipt-body');

			window.print();

			$('.pos').removeClass('print-hidden');
			$('.receipt').hide();
			$('.not-receipt').show();
	...
	pvm.pos_receipt = function() {
		$.post(Me.url_prefix+'lib/bg-frontdesk.php?get_request=pos-receipt', { id: pvm.selected().job_id }
		, function(data) {
			pvm.todays_date(data);
			$('.not-receipt').hide();
			$('.receipt').show();
			$('.not-pos').addClass('print-hidden');
			$('.main-container').addClass('print-receipt-body');

			window.print();

			$('.not-pos').removeClass('print-hidden');
			$('.receipt').hide();
			$('.not-receipt').show();
			$('.main-container').removeClass('print-receipt-body');
...
// Accept Refund Viewmodel
//-----------------------------------------------
function RefundViewmodel() {
	var rvm = this;

	// Observables
	rvm.amount	= ko.observable('');
	rvm.staff	= ko.observable('');
	rvm.purpose	= ko.observable('');

	// Event handlers
	rvm.accept_refund_form_submit = function() {
		if (rvm.staff == '') swal({title:'', text:"Please specify who is refunding", type: error});
		else if (rvm.purpose == '') swal({title:'', text:"Please specify what the refund is for", type: error});
		else if (rvm.amount == '' || rvm.amount < 1) swal({title:'', text:"Please enter valid refund amount", type: error});
		else return true;
	};
}
